...
cpdef tuple get_clusters(np.ndarray tree, dict stability,
    ...
    if cluster_selection_method == 'eom':
        for node in node_list:
            ...
            children = cluster_tree['child'][child_selection]
            subtree_stability = np.sum([stability[child] for child in children])
            sizes[node] = sum([sizes[child] for child in children]) if len(children) > 0 else 1

            if (max_number_of_joined_clusters is not None and sizes[node] > max_number_of_joined_clusters) or subtree_stability > stability[node]:
